# OpenKritt Web3 Bug-Bounty Workflow Pack v2.3
...
Version 2.3 restores the progressive fan-out used by OpenKritt's built-in workflows.
The v2.2 pack placed every hypothesis behind a global `consumeAll` deduplication job,
then four serial verification/PoC/program/report depths. That delayed every visible
finding and could discard valid candidates solely because a coarse triage pass did not
label them `high`. In v2.3:

- each hypothesis is independently verified and safely reproduced;
- each surviving candidate reaches the final finding depth while sibling work continues;
- technically supported candidates with a specific missing runtime fact remain visible
  as `needs_manual_validation`, never as verified/exploitable findings;
- OpenKritt's built-in post-processing performs global deduplication and ranking after
  workflow execution.
...
./kritt-headless import workflow \
  ./workflow-packs/web3-v2.3/05-defi-permissionless-economic-exploit-pipeline-v3.workflow.json
```
...
follow-up when the accepted impacts and custody model justify it. A workflow is a
blueprint, not an automatically scheduled scan: importing seven files does not launch
seven scans. Create one scan for each workflow that is relevant to the target.
