...
The model-catalog response is cached and refreshed by the engine. For DeepSeek,
OpenRouter, and xAI it contains searchable choices returned through the configured key.
DeepSeek accepts only a listed model. For OpenRouter and xAI, the list is a discovery
aid rather than an allowlist, so automation may still submit an exact custom model ID.
...
| `model` | Yes | Provider model ID or supported alias. DeepSeek requires a current catalog entry; OpenRouter and xAI also accept an exact custom model ID. |
| `model_provider` | Yes | `codex`, `claude`, `deepseek`, `openrouter`, or `xai`. |
...
`deepseek` + `codex`, `openrouter` + `claude-code` or advanced `codex`, and
`xai` + `grok-build`. The API
returns `422`
