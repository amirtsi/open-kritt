...
New browser profiles initially show **Codex** and **Claude Code**. Use **Show or hide
providers** in Accounts or beside a provider selector to reveal OpenRouter, xAI, or
DeepSeek. This preference stays in the browser; hidden configured accounts remain
listed in the manager, and an existing selected provider remains visible in its selector.
Hiding does not remove accounts or change saved selections.

[DeepSeek API checks](/ai-provider-setup/deepseek) let you save a key, discover models,
and run a short text check from Accounts. DeepSeek is not offered for scan execution
or draft generation.
...
<Steps>
  ...
  <Step title="Choose one access method">
    ...
    **OpenRouter API key**, or **xAI API key**. Follow the provider guide above to obtain
    ...
    device login, plus OpenRouter/xAI key management, are also available from the running
